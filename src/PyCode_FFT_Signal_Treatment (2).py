# -*- coding: utf-8 -*-
"""
Created on Wed Jul  9 13:48:13 2025
...
# REGRESSÃO LINEAR #
# Regressão linear usando TODOS os pontos entre o segundo máximo e o mínimo
# Baseado na curva FILTRADA (Fourier), não na média móvel
x_ini_r1, x_fim_r1 = sorted([x_segundo_max, x_minimo])
mask_r1 = (x >= x_ini_r1) & (x <= x_fim_r1)

# Corrigido: converter para numpy antes de reshape
X1 = x[mask_r1].to_numpy().reshape(-1, 1)
y1 = y_fft_filtrado[mask_r1]  
...
print(f"\nRegressão 1 (curva filtrada entre segundo máximo e mínimo):")
...
X2 = x_sub[start:end].to_numpy().reshape(-1, 1)
y2 = y_sub[start:end]
